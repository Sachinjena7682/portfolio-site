...
export default function Testimonials() {
  ...
  return (
    <section className="py-24 bg-surface/30 overflow-hidden" id="testimonials">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="text-4xl md:text-6xl font-black font-display mb-16">Direct <span className="text-primary">Impact</span></h2>
        <div className="flex gap-6 overflow-x-auto pb-8 snap-x no-scrollbar">
          {testimonials.map((t, i) => (
            <motion.div 
              ...
              className="min-w-[350px] md:min-w-[450px] snap-center glass-card p-10 rounded-3xl border border-white/5"
            >
              <div className="flex text-secondary mb-6">
                {[...Array(5)].map((_, i) => <Star key={i} className="w-5 h-5 fill-current" />)}
              </div>
              <p className="text-lg text-white italic mb-8">"{t.text}"</p>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-surface-high flex items-center justify-center border border-white/10">
